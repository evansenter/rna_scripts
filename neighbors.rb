module Rnabor
  class Neighbors
    ...
    def count_pairs(pair_hash)
      pair_hash.values.reject(&:nil?).count
    end
          ...
          (1..(sequence.length - base_pair_distance)).each do |i|
            ...
            (i..(j - MIN_LOOP_SIZE - 1)).each do |k|
              if can_pair?(k, j)
                ...
                sectional_pairs          = count_pairs(match_pairs(structure[((i - 1)..(j - 1))]))
                sectional_pairs_before_k = count_pairs(match_pairs(structure[((i - 1)..(k - 2))]))
                sectional_pairs_after_k  = count_pairs(match_pairs(structure[(k..(j - 2))]))
